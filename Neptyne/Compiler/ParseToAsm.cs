...
namespace Neptyne.Compiler
{
    ...
    {
        private static List<AsmDataVariable> _variables = new List<AsmDataVariable>();
        private static List<AsmFunction> _functions = new List<AsmFunction>();
        ...
        {
            _variables = new List<AsmDataVariable>();
            _functions = new List<AsmFunction>();
            _current = 0;

            AsmScript assemblyCode = new AsmScript();
                ...
                if (token.Type == ParserTokenType.AssignmentOperator && token.Value == "=")
                {
                    ...
                    else
                    {
                        throw new CompilerException(
                            $"Cannot convert expression of type '{GetVariableType(token.Type)}' to type '{GetVariableType(variableType)}'",
                            token.Line);
                    }
                }
        ...
        private static void DeclareVariable(string variableName, ParserToken token)
        {
            ...
            {
                AsmDataVariable variable = new AsmDataVariable($"{variableName}", token.Value, GetVariableType(token.Type));
        ...
        {
            AsmFunction function = new AsmFunction(variableName, type, token.Params, block);
        ...
        {
            List<AsmFunctionParameter> parameters = new List<AsmFunctionParameter>();
                ...
                if (token.Type == ParserTokenType.ValueType)
                {
                    ...
                    if (token.Type == ParserTokenType.Name)
                    {
                        if (GetVariableType(token.Type) == type)
                        {
                            ...
                            parameters.Add(new AsmFunctionParameter(type, value));
                        }
                        else
                        {
                            throw new CompilerException(
        ...
        {
            List<AsmStatement> statements = new List<AsmStatement>();
                    ...
                    switch (token.Value)
                    {
                        ...
                        case "out":
                            ...
                            if (token.Type == ParserTokenType.CallExpression)
                            {
                                ...
                                {
                                    throw new CompilerException($"Insufficient amount of parameters: 1 needed, {token.Params.Count} given",
                                        token.Line);
                                }
                            }
                            else
